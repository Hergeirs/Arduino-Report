...
\begin{itemize}
\item\citeurl{https://github.com/Hergeirs/Arduino2/tree/master/FWG7RQ3IRXT1DFL}{Arduino code}
\item \citeurl{https://github.com/Hergeirs/Arduino-Web}{Web Code}
\item \citeurl{https://github.com/Hergeirs/Arduino-Report}{IT Report}
\end{itemize}
...
We used the TMP36 temperature sensor, as it was supplied to use by the course administrators and it suits our purpose. The TMP36 is fairly simple. It can measure degrees from -40\textdegree{}C to \textdegree{125}C, where it has a precision\cite{tmpTutorial} of $\pm2$\textdegree{C}. But about $\pm1$\textdegree{C} in room temperature. The sensor outputs a voltage proportional to the vcc input voltage with an offset per degrees celsius. The output is linear to the temperature. Thus it is easy to calculate the temperature from the output voltage in an analog pin on the arduino.
...
The circuit for this temperature sensor can be seen in \ref{fig:TMP36GZCircuit}.
